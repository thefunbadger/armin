...
missing_vars = [
    ...
    if (var not in st.secrets and os.getenv(var) is None)
]
if missing_vars:
    st.error(
        f"Missing environment variables: {', '.join(missing_vars)}. Please set them before running the app."
    )
    st.stop()
...
def save_ai_insight_to_db(post_id, ai_text):
    collection = get_mongo_collection("auth")
    collection.update_one(
        {"post_id": post_id}, {"$set": {"ai_insight": ai_text}}, upsert=True
    )
...
def get_media_insights(access_token, media_id, media_type):
    try:
        ...
        if not metrics:
            st.session_state["api_errors"].append(
                f"Unsupported media type: {media_type}. Skipping insights."
            )
            return []
...
def fetch_all_data(access_token, instagram_account_id):
    ...
    if not media_items:
        st.warning(
            "No media items retrieved. Ensure your Instagram account has posts and the necessary permissions."
        )
    ...
    if not all_data:
        st.session_state["api_errors"].append(
            "No insights data fetched. Please check API permissions and Instagram Business account setup."
        )
...
def extract_hashtags(caption):
    hashtags = [
        tag.strip("#") for tag in caption.split() if tag.startswith("#")
    ]
...
def get_recommendations(df):
    ...
    if pd.notnull(avg_reach) and avg_reach < 1000:
        recommendations.append(
            "Your average reach is below 1,000. Consider optimizing your posting times and content."
        )
    elif pd.notnull(avg_reach):
        recommendations.append(
            "Your average reach is healthy. Keep up the good work!"
        )
    ...
    if (
        ...
        df["engagement_rate"] = (
            (df["likes"] + df["comments"]) / df["followers"].replace(0, 1) * 100
        )
        avg_engagement = df["engagement_rate"].mean()
        if pd.notnull(avg_engagement) and avg_engagement < 1:
            recommendations.append(
                "Your average engagement rate is below 1%. Engage more with your audience through interactive content."
            )
        elif pd.notnull(avg_engagement):
            recommendations.append(
                "Your engagement rate is healthy. Continue creating engaging content!"
            )
    ...
    if not top_hashtags.empty:
        ...
        recommendations.append(
            f"Try using the hashtag #{top_hashtag} more often to increase visibility."
        )
    ...
    if "timestamp" in df.columns:
        ...
        recommendations.append(
            f"Consider posting more frequently around {top_hour}:00 hours when your audience is most active."
        )
    ...
    def plot_reach_over_time(df):
        if "reach" in df.columns and not df["reach"].isnull().all():
            ...
            fig.update_layout(
                xaxis=dict(tickformat="%Y-%m-%d"), hovermode="x unified"
            )
            return fig
        else:
            st.warning(
                "Reach data is missing or incomplete. Unable to plot reach over time."
            )
            return None
    ...
    def plot_engagement_over_time(df):
        if (
            ...
            and not df["engagement_rate"].isnull().all()
        ):
            fig = px.line(
                df,
                x="timestamp",
            ...
            st.warning(
                "Engagement rate data is missing or incomplete. Unable to plot engagement rate over time."
            )
            return None
    ...
    def plot_top_posts(df, metric="reach", top_n=5):
        if metric not in df.columns or df[metric].isnull().all():
            st.warning(
                f"{metric.capitalize()} data is missing or incomplete. Unable to plot top posts by {metric}."
            )
            return None
        ...
        fig = px.bar(
            ...
            x="id",
            y=metric,
            title=f"Top {top_n} Posts by {metric.capitalize()}",
            labels={"id": "Post ID", metric: metric.capitalize()},
            template="plotly_dark",
        )
    ...
    def plot_comprehensive_metrics(df):
        ...
        for metric in metrics:
            if metric in df.columns and not df[metric].isnull().all():
                ...
                st.warning(
                    f"{metric.capitalize()} data is missing or incomplete. Unable to plot {metric} over time."
                )
        return figs
    ...
    def plot_follower_growth(df):
        if "followers" in df.columns and not df["followers"].isnull().all():
            ...
            st.warning(
                "Follower data is missing or incomplete. Unable to plot follower growth over time."
            )
            return None

    # AI Assistance Functions with Retry Logic
    def query_huggingface(
        prompt, model="distilgpt2", max_retries=5, backoff_factor=2
    ):
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if response.status_code == 200:
                ...
                st.warning(
                    f"AI Model is loading. Retrying in {backoff_factor} seconds... (Attempt {attempt}/{max_retries})"
                )
        ...
        return {
            "error": "Max retries exceeded. AI model is still loading or unavailable."
        }

    def ai_insight(
        selected_post, user_id, model, max_length, temperature, top_p
    ):
        ...
        hashtags_formatted = " ".join(
            [f"#{tag}" for tag in selected_post["hashtags"].split(",") if tag]
        )
        prompt = f"""
        ...
        response = query_huggingface(
            prompt, model=model, max_retries=5, backoff_factor=2
        )
    ...
    def compare_performance(df, access_token):
        ...
        competitors = st.text_input(
            "Enter Competitor Instagram Account IDs (comma separated):"
        )
        if competitors:
            ...
            if competitors_data:
                ...
                competitors_df = competitors_df.append(
                    user_metrics, ignore_index=True
                )
            ...
            st.info(
                "Enter competitor Instagram Account IDs to compare performance."
            )
    ...
    def display_visual_content_calendar(df):
        ...
        fig = px.timeline(
            df,
            x_start="scheduled_post_time",
            x_end="scheduled_post_time",
            y="media_type",
            title="Instagram Post Calendar",
            labels={"scheduled_post_time": "Time", "media_type": "Post Type"},
            template="plotly_dark",
        )
        fig.update_yaxes(categoryorder="total ascending")
        fig.update_layout(
            xaxis=dict(tickformat="%Y-%m-%d"), hovermode="closest"
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    def main():
        ...
        if "user_id" not in st.session_state:
            st.session_state["user_id"] = (
                "default_user"  # Replace with actual user identification
            )
                    ...
                    if not st.session_state["data_fetched"]:
                        with st.spinner("Loading data from database..."):
                            ...
                            if not df.empty:
                                ...
                                st.info(
                                    "No cached data found. Please update data to fetch from API."
                                )

                    if (
                        st.button("Update Data")
                        or not st.session_state["data_fetched"]
                    ):
                        with st.spinner("Fetching and processing data..."):
                            ...
                            if not pages:
                                st.error(
                                    "No Facebook Pages found. Ensure your account manages a page connected to Instagram."
                                )
                                return
                            ...
                            for page in pages:
                                ...
                                if instagram_account_id:
                                    st.session_state["instagram_account_id"] = (
                            ...
                            if "instagram_account_id" in st.session_state:
                                ...
                                user_instagram_id = st.session_state[
                                    "instagram_account_id"
                                ]

                                df = fetch_all_data(
                                    access_token, user_instagram_id
                                )
                                if not df.empty:
                                    df = calculate_metrics(df)
                                    st.session_state["df"] = df
        ...
        if not st.session_state["df"].empty:
            ...
            with st.sidebar:
                ...
                selected_media_types = st.multiselect(
                    "Select Media Types", media_types, default=media_types
                )
                ...
                ai_model = st.selectbox(
                    "Select AI Model",
                    options=["distilgpt2", "gpt2", "facebook/opt-125m"],
                )
                ai_max_length = st.slider(
                    "Max Length", min_value=50, max_value=500, value=300
                )
            ...
            if selected_hashtags:
                filtered_df = filtered_df[
                    filtered_df["hashtags"].str.contains(
                        "|".join(selected_hashtags), na=False
                    )
                ]

            st.write(
                f"Displaying {len(filtered_df)} out of {len(df)} posts based on selected filters."
            )
            ...
            with tabs[0]:
                ...
                with col2:
                    ...
                    if fig_engagement:
                        st.plotly_chart(
                            fig_engagement, use_container_width=True
                        )
                ...
                with col4:
                    ...
                    if fig_top_hashtags:
                        st.plotly_chart(
                            fig_top_hashtags, use_container_width=True
                        )
            ...
            with tabs[2]:
                ...
                selected_post_id = st.selectbox(
                    "Select a Post ID to Get AI Insights", options=post_ids
                )

                selected_post = filtered_df[
                    filtered_df["id"] == selected_post_id
                ].iloc[0]
            ...
            with tabs[3]:
                compare_performance(
                    filtered_df, st.session_state["access_token"]
                )
        ...
        if "access_token" not in st.session_state:
            ...
            st.markdown(
                f'<a href="{authorization_url}">Login with Facebook</a>',
            ...
            if "code" in query_params:
                ...
                if token:
                    long_lived_token, expires_in = (
                        exchange_for_long_lived_token(token["access_token"])
                    )
                    if long_lived_token:
                        ...
                        st.session_state["api_errors"].append(
                            "Failed to obtain a long-lived access token."
                        )
                else:
                    st.session_state["api_errors"].append(
